#!/usr/bin/env python3
import os, sys, struct, json, math, shutil, argparse, subprocess, zipfile
...
# ---------------------------------------------------------
#  BASIC UTILITIES + FULL OVERWRITE
# ---------------------------------------------------------
def full_overwrite_dirs():
    for p in [
        SRC_PUBLIC / "sounds",
        SRC_PUBLIC / "video",
        SRC_PUBLIC / "models" / "fighters",
        SRC_PAGES,
        DIST,
    ]:
        if p.exists():
...
def make_box_glb(path, size, color):
    ...
    Pure Python. No external GLB libs.
    """
    w, h, d = size
    r, g, b = color
...
def main():
    args = parse_args()
    print("=== DancePlanet Full Generator (Pure-Python GLB) ===")

    print("[0] Full overwrite of generated folders...")
    full_overwrite_dirs()
    ensure_dirs()

    print("\n[1] Generating GLB fighters...")
    make_fighter_glbs()
    write_fighter_lore()
    ...
    print("\n[3] Generating videos (this may take a bit)...")
    ...
    print("\n[5] Building Vercel deployment folder...")
    build_vercel_dist()

    print("\n=== Build Complete ===")
    print("Project root:      ", ROOT.resolve())
    print("Vercel package:    ", DIST.resolve())
    print("Deploy via Vercel with Root Directory = 'vercel_dist'.")
